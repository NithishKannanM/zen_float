...
> **C1 amendment (no-move invariant) — [Confirmed against Glance source].** The nested `<browser>` is **never reparented** in the DOM (moving a remote browser destroys/recreates its frameloader → docshell/page-state loss). Instead, exactly like Glance's `.zen-glance-overlay`, ZF-020 applies a class (`.zen-float-browser`) to the float tab's **own** `.browserSidebarContainer`; its inner `.browserContainer` is set `position: fixed`. `.zen-float-overlay` is a **chrome frame only** (title bar / handles), never a browser parent. Frame and browser-container share one set of `--zen-float-*` geometry CSS variables, so they align without imperative syncing. Glance CSS confirms the pattern: `.browserSidebarContainer.zen-glance-overlay { visibility: inherit; overflow: visible }` + `… .browserContainer { position: fixed }`.
