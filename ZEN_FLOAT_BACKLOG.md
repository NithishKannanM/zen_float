...
- **AC status — SATISFIED by ZF-023 (target presets & switching):** "switching target reloads browser" ✅ (`FloatWindow.switchTarget()` → `BrowserHost.navigate()`, reusing the same tab/browser — no respawn) and "custom URL accepted/validated (http/https)" ✅ (`TargetPresets.validate()`, scheme allowlist, `Services.uriFixup`). Presets landed as **`TargetPresets`** — a separate class, since `TargetRegistry` here means the metadata layer. Extensible descriptor `{type:"web"}` carried per EDD §13. Evidence: `reviews/ZF-023-VALIDATION.md`, `reviews/evidence/zf023-picker.png`.

### ZF-023 — Embedding fallback (open-in-tab)
> ⚠ **Number collision:** the *implementation* ticket labelled "ZF-023" delivered **target presets & switching** (the ZF-022 leftovers) — see the ZF-022 entry above and `reviews/ZF-023-VALIDATION.md`. The embedding-fallback work below is **still unbuilt** and needs renumbering (suggest ZF-024) when it is scheduled.
